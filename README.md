# 🎬 抖音广告视频 - 本地视频混剪 / 可灵 AI 视频生成

> **一句话说明**：选择“本地视频混剪”或“可灵 AI 视频生成”作为互斥的画面来源，再由同一套智能后期完成带货脚本、单条口播、字幕、BGM、转场、特效、CTA、调色与最终导出。
...
> **本地视频混剪**不需要配置可灵 Key。需要 LLM 生成文案或豆包 TTS 时，再配置对应服务。
>
> **可灵 AI 视频生成**必须配置一种可灵鉴权。获取 Key：可灵 → [开放平台](https://app.klingai.com/cn/dev)，豆包 TTS → [火山引擎语音控制台](https://console.volcengine.com/speech/new/setting/apikeys)。
...
看到以下问题时直接回车，默认选择 `y`，再输入素材文件夹路径：

```text
是否使用本地视频素材文件夹？(y/n) [y]：
```

本地流程以视频素材为画面事实来源：先分析画面语义、时间窗口、运动、构图、亮度、色彩和音频特征，再生成带货脚本与全视频唯一口播，随后按脚本语义选择素材并完成后期。目标时长是偏好，最终时间轴服从素材的可用内容和自然叙事，不调用可灵生成新画面。
...
```text
是否使用本地视频素材文件夹？(y/n) [y]：n
```

AI 流程会根据产品信息生成脚本、角色与商品视觉约束，通过可灵图片/视频 API 生成分镜候选并自动择优，然后进入共享后期。执行前会显示可灵成本估算；`--mode`、`--duration`、`--best-of`、图片先行和低成本策略只影响此模式。

#### 两种模式共享的后期能力

1. 基于产品信息和画面证据生成带货脚本、字幕与单条口播
2. 根据内容和视听特征智能选择 BGM、音量、转场、特效与 CTA
3. 视频稳定化、去闪烁、调色、字幕安全区和音画同步
4. 黑帧、冻结帧、清晰度、音频与合规检测
5. 导出抖音标准成片、封面、标题和话题标签
...
│ 选择互斥的画面来源                                        │
├──────────────────────────┬───────────────────────────────┤
│ A. 本地视频混剪          │ B. 可灵 AI 视频生成/混剪      │
│ 本地素材目录             │ 产品信息 + 可选商品参考图     │
│          ↓               │              ↓                │
│ 多模态素材理解与打标     │ 带货脚本与视觉约束            │
│          ↓               │              ↓                │
│ 基于素材生成脚本与口播   │ 角色定妆照 / 图片先行预检     │
│          ↓               │              ↓                │
│ 按脚本语义智能选择片段   │ 可灵分镜生成 / best-of 择优   │
│                          │                                │
│ 不调用可灵图片/视频 API  │ 调用可灵图片/视频 API         │
└─────────────┬────────────┴───────────────┬───────────────┘
              └──────────────┬─────────────┘
                             ▼
┌─────────────────────────────┐
...
```bash
python one_click_create.py \
  --local-assets "/Users/your-name/Videos/product-assets" \
  --voiceover \
  --target-duration 15
```
...
```text
🎞️ 本地视频混剪计划
   素材目录：/Users/your-name/Videos/product-assets
   处理链路：素材理解 → 带货脚本与单条口播 → 智能选片 → 后期合成
   时长偏好：15s（最终时长由素材自然叙事决定）
   可灵图片/视频生成：不调用
```

### 示例 2：可灵 AI 美妆视频生成/混剪
...
4. 两个分支汇合后，由 `video_merger.py` 完成拼接、智能转场、卡点、字幕和调色
5. 调用 `bgm_client.py` 根据视听合同智能选曲和控制音量
6. 调用 `tts_client.py` 生成全视频唯一 TTS 口播母带
7. 调用 `quality_checker.py` 和 `compliance_checker.py` 执行质量与合规检测
8. 生成封面、品牌水印、标题和话题标签并导出最终成片
